...
function simpleBomBuilderFixtures(): array
{
    ...
    foreach ([
        ...
        'Caustic Soda' => 'kg',
        'Borax' => 'kg',
        'Quick BOM Finished Carton' => 'pcs',
    ] as $name => $unit) {
        $materials[$name] = DB::table('products')->insertGetId([
            'name' => $name . '-' . uniqid(),
    ...
    // The adhesive calculator resolves materials by canonical names, so make
    // the temporary raw products use those names after their unique insert.
    foreach (['Fluting', 'Kraft Liner', 'Corn Flour', 'Seligate (Glue)', 'Caustic Soda', 'Borax'] as $canonical) {
        DB::table('products')->where('id', $materials[$canonical])->update(['name' => $canonical]);
    }
