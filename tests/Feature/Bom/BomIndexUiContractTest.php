...
it('keeps the BOM library on the simplified server-driven index UI', function () {
    ...
    expect($view)
        ...
        ->toContain('@media (max-width: 1499.98px)')
        ->toContain('class="bom-responsive-row"')
        ->toContain('<col class="col-number">')
        ->toContain('<col class="col-actions">')
        ->toContain('.bom-index .table-ledger col.col-number { width: 44px; }')
