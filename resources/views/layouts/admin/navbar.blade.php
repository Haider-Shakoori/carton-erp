                ...
                <li class="nav-item dropdown-notifications navbar-dropdown dropdown me-xl-1 me-4">
                    <a class="nav-link btn btn-text-secondary rounded-pill btn-icon dropdown-toggle hide-arrow"
                        ...
                        @if($navbarUnreadCount > 0)
                            <span class="position-absolute start-50 translate-middle-y badge badge-dot bg-danger top-0 mt-2 border"></span>
                        @endif
                    </a>
                        ...
                        <li class="dropdown-notifications-list scrollable-container">
                            <ul class="list-group list-group-flush">
                                @forelse($navbarNotifications as $navbarNotification)
                                    @php
                                        $navbarData = $navbarNotification->data;
                                        $navbarSeverity = $navbarData['severity'] ?? 'normal';
                                        $navbarBadge = match($navbarSeverity) {
                                            'critical' => 'danger',
                                            'high' => 'warning',
                                            default => 'primary',
                                        };
                                    ...
                                    <li class="list-group-item list-group-item-action dropdown-notifications-item {{ $navbarNotification->read_at ? 'marked-as-read' : '' }}">
                                        <a href="{{ route('admin.stock-notifications.open', $navbarNotification) }}"
                                           class="text-decoration-none text-body d-block">
                                            <div class="d-flex gap-3">
                                                <div class="flex-shrink-0">
                                                    <span class="avatar-initial rounded-circle bg-label-{{ $navbarBadge }} d-flex align-items-center justify-content-center"
                                                          style="width: 38px; height: 38px;">
                                                        <i class="ri-error-warning-line"></i>
                                                    </span>
                                                </div>
                                                <div class="flex-grow-1 overflow-hidden">
                                                    <h6 class="small mb-1 text-truncate">{{ $navbarData['title'] ?? 'Stock Control Notification' }}</h6>
                                                    <small class="d-block text-body text-truncate">{{ $navbarData['message'] ?? '' }}</small>
                                                    <small class="text-muted">{{ $navbarNotification->created_at?->diffForHumans() }}</small>
                                                </div>
                                                @if(! $navbarNotification->read_at)
                                                    <span class="badge badge-dot bg-primary mt-2"></span>
                                                @endif
                                            </div>
                                        </a>
                                    </li>
