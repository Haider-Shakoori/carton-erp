...
@section('css')
    ...
    <style>
        ...
        .stat-card.green::before { background: linear-gradient(90deg, var(--primary-color), #34d399); }
...
<div class="container-fluid px-3 px-md-4">
    ...
    <div class="page-header">
        <div>
            <h1>
                ...
                {{ __('ui.supplier') }} <span class="accent">{{ __('ui.details') }}</span>
            </h1>
            <p class="subtitle">
    ...
    <div class="stats-grid">
        <div class="stat-card purple">
            ...
            <div class="stat-label">{{ __('ui.supplier_name') }}</div>
        </div>
    ...
    <div class="info-card">
        <div class="info-grid">
            <div class="info-item">
                <div class="label">{{ __('ui.supplier_code') }}</div>
                <div class="value">{{ $supplier->code }}</div>
            </div>
            <div class="info-item">
                <div class="label">{{ __('ui.contact') }}</div>
                <div class="value">{{ $supplier->contact ?? 'N/A' }}</div>
            </div>
            <div class="info-item">
                <div class="label">{{ __('ui.email') }}</div>
                <div class="value">{{ $supplier->email ?? 'N/A' }}</div>
            </div>
            <div class="info-item">
                <div class="label">{{ __('ui.whatsapp') }}</div>
                <div class="value">{{ $supplier->whatsapp ?? 'N/A' }}</div>
            </div>
            <div class="info-item">
                <div class="label">{{ __('ui.company') }}</div>
                <div class="value">{{ $supplier->company ?? 'N/A' }}</div>
            </div>
            <div class="info-item">
        ...
        <div class="mt-2">
            ...
            <div class="value" style="font-weight: 400;">{{ $supplier->notes }}</div>
        </div>
        @endif
    </div>
    ...
    <div class="card-header">
        <h5>
            ...
            <span class="badge bg-secondary ms-2" style="font-size: 0.65rem; font-weight: 600;">
                {{ $supplier->transactions->count() ?? 0 }} transactions
            </span>
        </h5>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-hover table-sm" id="transactions-table" style="width: 100%;">
                ...
                <tbody>
                    @forelse($supplier->transactions->sortByDesc('created_at') as $transaction)
                        <tr>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-4">
                                ...
                                <span class="text-muted">No transactions found for this supplier</span>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('exportImage').addEventListener('click', function() {
                ...
                html2canvas(exportArea, {
                    ...
                    link.download = '{{ $supplier->code }}-summary.png';
            ...
            document.getElementById('exportPdf').addEventListener('click', function() {
                ...
                html2canvas(exportArea, {
                    ...
                    pdf.save('{{ $supplier->code }}-summary.pdf');
