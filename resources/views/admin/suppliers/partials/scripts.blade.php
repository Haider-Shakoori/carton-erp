...
<script>
    $(function() {
        const table = $('#yajra-datatable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('admin.suppliers.fetch-suppliers') }}",
                data: function(d) {
        ...
        $(document).on('click', '.btn-send-whatsapp', function() {
            ...
            Swal.fire({
                title: 'Send WhatsApp?',
                text: 'Do you want to send a message to this saraf/bank?',
                icon: 'question',
        ...
        $(document).on('click', '.btn-edit-account', function(e) {
            ...
            fetch(`/admin/accounts/${id}/edit`)
                ...
                .then(data => {
                    ...
                    $('#create_account-sub-category').val(data.account_sub_category_id);
        ...
        function fetchStats() {
            $.get("{{ route('admin.suppliers.update-stats') }}", {
                ...
                $('#total_credit').text(
                    `${data.currency} ${Number(data.total_credit.replace(/,/g, '')).toLocaleString()}`
                );
                $('#total_debit').text(
                    `${data.currency} ${Number(data.total_debit.replace(/,/g, '')).toLocaleString()}`
                );
        ...
        function checkCodeAvailability() {
            ...
            if (!code) return;
            feedback.innerHTML =
                `<span class='text-muted'><span class='spinner-border spinner-border-sm'></span> {{ __('ui.checking') }}</span>`;
