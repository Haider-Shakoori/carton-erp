...
@section('css')
    ...
    <style>
        :root {
            ...
            --agent-gray-50: #f8fafc;
            --agent-gray-100: #f1f5f9;
            --agent-gray-200: #e2e8f0;
            --agent-gray-300: #cbd5e1;
            --agent-gray-400: #94a3b8;
            --agent-gray-500: #64748b;
            --agent-gray-600: #475569;
            --agent-gray-700: #334155;
            --agent-gray-800: #1e293b;
            --agent-gray-900: #0f172a;
            --agent-radius: 16px;
            --agent-radius-sm: 10px;
            --agent-shadow: 0 1px 3px rgba(0,0,0,0.02), 0 8px 32px rgba(0,0,0,0.04);
            --agent-shadow-hover: 0 1px 3px rgba(0,0,0,0.02), 0 12px 48px rgba(0,0,0,0.08);
        }
        ...
        .page-header h1 .accent {
            background: linear-gradient(135deg, var(--agent-primary), #7c3aed);
        ...
        .stat-card {
            ...
            border: 1px solid var(--agent-gray-200);
        ...
        .main-card .card-header {
            ...
            border-bottom: 1px solid var(--agent-gray-200);
            display: flex;
            justify-content: space-between;
            align-items: center;
            flex-wrap: wrap;
        ...
        .filter-bar .btn-outline-custom:hover {
            background: var(--agent-gray-50);
            border-color: var(--agent-gray-300);
        }
        ...
        .table-custom thead th {
            ...
            border-bottom: 2px solid var(--agent-gray-200);
            white-space: nowrap;
            text-align: left;
        }
        .table-custom tbody td {
            padding: 0.625rem 0.75rem;
            border-bottom: 1px solid var(--agent-gray-100);
        ...
        .agent-cell .info .name {
            font-weight: 600;
            color: var(--agent-gray-800);
            font-size: 0.875rem;
        }
        .agent-cell .info .meta {
            font-size: 0.6875rem;
            color: var(--agent-gray-400);
            display: flex;
            gap: 0.5rem;
        ...
        .agent-cell .info .meta .badge-code {
            background: var(--agent-gray-100);
        ...
        .pagination-wrap {
            ...
            gap: 0.75rem;
            padding-top: 1rem;
            border-top: 1px solid var(--agent-gray-200);
            margin-top: 1rem;
        }
...
@section('content')
    ...
    <div class="container-fluid px-3 px-md-4">
        ...
        <div class="page-header">
            <div>
                <h1>
                    <i class="bi bi-person-badge" style="color: var(--agent-primary);"></i>
                    <span class="accent">{{ __('ui.agents') }}</span>
                </h1>
                <p class="subtitle">
                    <i class="bi bi-people me-1"></i>
                    Manage agents, track balances, and monitor transactions
                </p>
            </div>
        </div>
        ...
        <div class="stats-grid">
            <div class="stat-card purple">
                <div class="stat-top">
                    ...
                    <div class="stat-value" id="total_agents">{{ number_format($stats['total_agents'] ?? 0) }}</div>
                </div>
            ...
            <div class="stat-card blue">
                <div class="stat-top">
                    ...
                    <div class="stat-value" id="balance_value"
                        style="color: {{ ($stats['balance'] ?? 0) >= 0 ? 'var(--agent-success)' : 'var(--agent-danger)' }};">
        ...
        <div class="main-card">
            <div class="card-header">
                <h5>
                    <i class="bi bi-list-ul" style="color: var(--agent-primary);"></i>
                    Agent Directory
                    <span class="badge-count">{{ $agents->total() }} agents</span>
                </h5>
            ...
            <div class="card-body">
                ...
                <div class="filter-bar">
                    <div class="filter-group">
                        ...
                        <input type="text" id="searchInput" placeholder="{{ __('ui.search_agents') }}"
                    ...
                    @if (request('search') || request('currency_id'))
                        <a href="{{ route('admin.agents.index') }}" class="btn-outline-custom">
                            <i class="bi bi-arrow-counterclockwise"></i> {{ __('ui.reset') }}
                        </a>
                    @endif

                    @can('create agents')
                        <button class="btn-primary-custom" style="margin-left: auto;" data-bs-toggle="modal" data-bs-target="#createAgentModal">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="agent-cell">
                                            <div class="avatar">{{ substr($agent->name, 0, 2) }}</div>
                                            <div class="info">
                                                <div class="name">{{ $agent->name }}</div>
                                                <div class="meta">
                                                    @if ($agent->company)
                                                        <span><i class="bi bi-building"></i> {{ $agent->company }}</span>
                                                    @endif
                                                    @if ($agent->email)
                                                        <span><i class="bi bi-envelope"></i> {{ $agent->email }}</span>
                                                    @endif
                                                    <span class="badge-code">{{ $agent->code }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <span style="font-family: monospace; font-weight: 600; color: var(--agent-primary);">
                                            {{ $agent->code }}
                                        </span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="contact-details">
                                            ...
                                            @endif
                                            @if (!$agent->contact && !$agent->whatsapp)
                                    ...
                                    <td class="text-end">
                                        <div class="action-buttons">
                                            <a href="{{ route('admin.agents.show', $agent->id) }}" class="btn-action view" title="{{ __('ui.view_details') }}">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                            @can('update agents')
                                                <button class="btn-action edit btn-edit-agent" data-id="{{ $agent->id }}" title="{{ __('ui.edit') }}">
                                                    <i class="bi bi-pencil"></i>
                                                </button>
                                            @endcan
                                            @can('delete agents')
                                                <button class="btn-action delete btn-delete-agent" data-id="{{ $agent->id }}" data-name="{{ $agent->name }}" title="{{ __('ui.delete') }}">
                                                    <i class="bi bi-trash3"></i>
                                                </button>
                                            @endcan
                                            @if ($agent->whatsapp)
                                                <button class="btn-action whatsapp btn-send-whatsapp" data-url="{{ route('admin.agents.send-whatsapp', $agent->id) }}" title="{{ __('ui.send_whatsapp') }}">
                                ...
                                <tr>
                                    <td colspan="7">
                                        <div class="empty-state">
                                            ...
                                            <div class="title">No agents found</div>
                                            <div class="subtitle">Create your first agent by clicking "New Agent"</div>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
    ...
    <div class="modal fade" id="createAgentModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header" style="background: linear-gradient(135deg, #4f46e5, #7c3aed); color: white; border: none;">
                    <h5 class="modal-title">
                        <i class="bi bi-person-plus me-2"></i> New Agent
                    </h5>
                ...
                <form id="createAgentForm">
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label">{{ __('ui.agent_name') }} <span class="text-danger">*</span></label>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <span class="input-group-text" style="background: var(--agent-gray-50); font-weight: 600; color: var(--agent-primary);">AGT-</span>
                                    <input type="text" class="form-control" name="code" id="create_agent_code" placeholder="0001" required>
                                    <button class="btn btn-outline-secondary" type="button" id="generateAgentCodeBtn" title="{{ __('ui.generate_code') }}">
                                        <i class="bi bi-arrow-repeat"></i>
                                    </button>
                                </div>
                                <small id="create_code_feedback" class="text-muted">Code will be auto-generated with AGT- prefix</small>
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" name="whatsapp" placeholder="+93 700 000 000">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">{{ __('ui.email') }}</label>
                                <input type="email" class="form-control" name="email" placeholder="agent@example.com">
                            </div>
                            <div class="col-md-6">
                    ...
                    <div class="modal-footer" style="background: var(--agent-gray-50); border-top: 1px solid var(--agent-gray-200);">
                        <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">
                            <i class="bi bi-x-lg"></i> {{ __('ui.cancel') }}
                        </button>
                        <button type="submit" class="btn btn-primary" id="submitAgentBtn">
            ...
            <div class="modal-content">
                ...
                <form id="editAgentForm">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <small id="edit_code_feedback" class="text-muted">Full code will be AGT-XXXX</small>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label">{{ __('ui.contact') }}</label>
                    ...
                    <div class="modal-footer" style="background: var(--agent-gray-50); border-top: 1px solid var(--agent-gray-200);">
                        ...
                        <button type="submit" class="btn btn-warning" id="updateAgentBtn">
                            <i class="bi bi-check-lg"></i> Update Agent
                        </button>
                    </div>
                </form>
            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function checkCodeAvailability(code, feedbackElement, submitBtn) {
                if (!code) {
                    $(feedbackElement).text('Enter a code').removeClass('text-success text-danger').addClass('text-muted');
                    return;
                }

                var checkCode = code;
                if (!checkCode.startsWith('AGT-')) {
                    checkCode = 'AGT-' + checkCode;
                }

                $.ajax({
                    url: '{{ route("admin.agents.check-code") }}',
                    method: 'POST',
                    data: {
                        code: checkCode,
                        _token: '{{ csrf_token() }}'
                    ...
                    success: function(response) {
                        if (response.available) {
                            $(feedbackElement).text('✅ Code available: ' + response.code).removeClass('text-danger')
                    ...
                    error: function() {
                        $(feedbackElement).text('⚠️ Error checking code').removeClass('text-success text-danger')
            ...
            $('#generateAgentCodeBtn').on('click', function() {
                var code = generateAgentCode();
                var suffix = code.replace('AGT-', '');
                $('#create_agent_code').val(suffix);
                $('#create_code_feedback').text('Generated: ' + code).removeClass('text-muted text-danger')
                    .addClass('text-success');
                checkCodeAvailability(code, '#create_code_feedback', '#submitAgentBtn');
            });

            // ─── Create Code Input Change ───
            $('#create_agent_code').on('input', function() {
                var val = $(this).val().trim();
                var fullCode = 'AGT-' + val;
                if (val) {
                    checkCodeAvailability(fullCode, '#create_code_feedback', '#submitAgentBtn');
                } else {
                    $('#create_code_feedback').text('Enter a code suffix').removeClass('text-success text-danger')
                        .addClass('text-muted');
            ...
            $('#edit_code').on('input', function() {
                ...
                var fullCode = 'AGT-' + val;
                if (val) {
                    checkCodeAvailability(fullCode, '#edit_code_feedback', '#updateAgentBtn');
                } else {
                    $('#edit_code_feedback').text('Enter a code suffix').removeClass('text-success text-danger')
                        .addClass('text-muted');
                    $('#updateAgentBtn').prop('disabled', false);
                }
            });

            // ─── Auto-generate code on modal open ───
            $('#createAgentModal').on('shown.bs.modal', function() {
                if (!$('#create_agent_code').val()) {
                    setTimeout(function() {
                        $('#generateAgentCodeBtn').click();
                    }, 300);
                }
            });
            ...
            $('#applyFilters').on('click', function() {
                ...
                var url = '{{ route("admin.agents.index") }}';
                var params = [];
            ...
            $('#createAgentForm').on('submit', function(e) {
                ...
                var $btn = $('#submitAgentBtn');
                var codeSuffix = $('#create_agent_code').val().trim();
                
                if (!codeSuffix) {
                    Swal.fire('Error', 'Please enter or generate an agent code.', 'error');
                    return;
                }
                
                var fullCode = 'AGT-' + codeSuffix;
                
                // Check if code is available
                var isCodeAvailable = $('#create_code_feedback').hasClass('text-success');
                if (!isCodeAvailable) {
                    Swal.fire('Error', 'Please use a valid and available agent code.', 'error');
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $btn.prop('disabled', false).html('<i class="bi bi-check-lg"></i> Create Agent');
                        }
                    },
                    error: function(xhr) {
            ...
            $(document).on('click', '.btn-edit-agent', function() {
                ...
                $.get('/admin/agents/' + id + '/edit', function(data) {
                    ...
                    $('#edit_code_feedback').text('Editing: AGT-' + code).removeClass('text-muted text-danger')
                        .addClass('text-success');
            ...
            $('#editAgentForm').on('submit', function(e) {
                ...
                var id = $('#edit_agent_id').val();
                var $btn = $('#updateAgentBtn');
                var codeSuffix = $('#edit_code').val().trim();
                
                if (!codeSuffix) {
                    Swal.fire('Error', 'Please enter a valid agent code.', 'error');
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            Swal.fire('Error', response.message, 'error');
                            $btn.prop('disabled', false).html('<i class="bi bi-check-lg"></i> Update Agent');
                        }
                    },
                    error: function(xhr) {
                        var errors = xhr.responseJSON?.errors;
            ...
            // ─── Preview Agent ───
            $(document).on('click', '.btn-preview-agent', function() {
                var id = $(this).data('id');
                ...
                $.get('/admin/agents/' + id + '/print', function(data) {
            ...
            $(document).on('click', '.btn-send-whatsapp', function() {
                ...
                Swal.fire({
                    title: 'Send WhatsApp?',
                    text: 'Send a message to this agent via WhatsApp?',
                    icon: 'question',
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        $.get(url, function(response) {
                            Swal.fire('Success!', response.message || 'Message sent successfully.', 'success');
                        }).fail(function(xhr) {
            ...
            $('#createAgentModal').on('hidden.bs.modal', function() {
                ...
                $('#create_code_feedback').text('Code will be auto-generated with AGT- prefix').removeClass('text-success text-danger').addClass('text-muted');
                $('#submitAgentBtn').prop('disabled', false).html('<i class="bi bi-check-lg"></i> Create Agent');
            });

            $('#editAgentModal').on('hidden.bs.modal', function() {
                $('#editAgentForm')[0].reset();
                $('#edit_code_feedback').text('Full code will be AGT-XXXX').removeClass('text-success text-danger').addClass('text-muted');
                $('#updateAgentBtn').prop('disabled', false).html('<i class="bi bi-check-lg"></i> Update Agent');
            });
