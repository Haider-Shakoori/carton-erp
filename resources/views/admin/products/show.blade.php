...
@section('content')
    <div class="container-fluid px-3 px-md-4">
        ...
        <div class="table-card" style="margin-bottom: 30px;">
            <div class="card-header-custom">
                ...
                <div class="d-flex gap-2">
                    ...
                    @if (strtolower($product->unit ?? '') === 'roll' && ($product->default_kg_per_roll ?? 0) > 0)
                        <span class="header-badge" style="background: #fef3c7; color: #92400e;">
                            <i class="bi bi-weight me-1"></i> {{ number_format($product->default_kg_per_roll, 2) }} kg / roll
                        </span>
                    @endif
                </div>
            </div>
            <div style="padding: 20px;">
                <div class="row align-items-center">
                    <div class="col-auto">
                        ...
                        @else
                            <div
                                style="width: 100px; height: 100px; border-radius: 12px; background: var(--bg-secondary); display: flex; align-items: center; justify-content: center; font-size: 40px; color: var(--text-muted); border: 2px solid var(--border-color);">
                    ...
                    <div class="col">
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 16px; margin-top: 8px;">
                            <span
                                style="display: flex; align-items: center; gap: 4px; color: var(--text-secondary); font-size: 14px;">
                                <i class="bi bi-hash" style="color: var(--primary);"></i> SKU: {{ $product->sku ?? 'N/A' }}
                            </span>
                            <span
                                style="display: flex; align-items: center; gap: 4px; color: var(--text-secondary); font-size: 14px;">
                                <i class="bi bi-calendar3" style="color: var(--primary);"></i> Added:
        ...
        {{-- ============================================================
        BATCH DETAILS TABLE - WITH PURCHASE ORDER NUMBER
        ============================================================ --}}
        <div class="table-card">
            ...
            <div class="table-responsive-custom">
                <table class="table-ledger">
                    <thead>
                        <tr>
                            <th style="min-width: 140px;">{{ __('ui.po_number') }}</th>
                    ...
                    <tbody>
                        @forelse($batchMetrics as $batch)
                            <tr>
                                <td>
                                    <a href="{{ route('admin.purchase-orders.show', $batch['purchase_id']) }}"
                                        style="font-weight: 600; color: var(--primary); text-decoration: none; display: inline-flex; align-items: center; gap: 4px;"
                                        title="{{ __('ui.view_purchase_details') }}">
                                ...
                                <td>
                                    <div class="num-cell text-center">
                                        ...
                                        @if (!empty($batch['is_roll_batch']) && $batch['kg_per_roll'] > 0)
                                            <div style="font-size: 11px; color: var(--text-secondary);">
                                                × {{ number_format($batch['kg_per_roll'], 0) }} kg = {{ number_format($batch['total_weight_kg'], 2) }} kg
                                            </div>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td class="text-center">
                                    @if ($batch['qty_available'] > 0)
                                        ...
                                        @if (!empty($batch['is_roll_batch']) && $batch['qty_kg_available'] > 0)
                                            <div style="font-size: 11px; color: var(--text-secondary); margin-top: 2px;">
                                                {{ number_format($batch['qty_kg_available'], 4) }} kg
                                            </div>
                                        @endif
                                    @else
                                ...
                                <td>
                                    <div class="num-cell text-end" style="justify-content: flex-end;">
                                        ...
                                        @if (!empty($batch['is_roll_batch']) && $batch['landed_cost_per_kg'] > 0)
                                            <div style="font-size: 11px; color: var(--text-secondary);">
                                                = ${{ number_format($batch['landed_cost_per_kg'], 2) }}/kg
                                            </div>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td>
                                    <div style="display: flex; align-items: center; gap: 8px;">
                                        <div
                                            style="flex: 1; height: 6px; background: var(--bg-secondary); border-radius: 10px; overflow: hidden; min-width: 60px;">
                                            <div
                                                style="height: 100%; width: {{ min($batch['utilization_rate'], 100) }}%; background: {{ $batch['utilization_rate'] > 80 ? 'var(--success)' : ($batch['utilization_rate'] > 50 ? 'var(--warning)' : 'var(--danger)') }}; border-radius: 10px; transition: width 0.3s ease;">
                                            </div>
                                        </div>
                ...
                <div class="table-card">
                    <div class="card-header-custom">
                        <h5>
                            ...
                            <span class="header-badge">{{ __('ui.last_30_days') }}</span>
                        </h5>
                    </div>
                            ...
                            <div
                                ...
                                <div>
                                    @if ($activity->qty_sold > 0)
                                        ...
                                        <span
                                            style="color: var(--text-primary); font-weight: 600;">{{ number_format($activity->qty_sold, 2) }}</span>
                                    @elseif($activity->qty_wasted > 0)
                                        <span style="color: var(--danger); font-weight: 600;">
                                            <i class="bi bi-trash"></i> {{ __('ui.wasted') }}
                                        </span>
                                        <span
                                            style="color: var(--text-primary); font-weight: 600;">{{ number_format($activity->qty_wasted, 2) }}</span>
                                    @else
                                        <span style="color: var(--primary); font-weight: 600;">
                                            <i class="bi bi-arrow-down-circle"></i> {{ __('ui.purchased') }}
                                        </span>
                                        <span
                                            style="color: var(--text-primary); font-weight: 600;">{{ number_format($activity->qty, 2) }}</span>
                                    @endif
                                    <span style="color: var(--text-muted); font-size: 13px; margin-left: 8px;">
                                        PO: {{ $activity->purchase->purchase_order_no ?? 'N/A' }}
                                    </span>
                                    <span style="color: var(--text-muted); font-size: 13px; margin-left: 4px;">
                ...
                <div class="table-card">
                    ...
                    <div style="padding: 16px;">
                        @php
                            ...
                            $activeBatches = $batchMetrics
                                ->filter(function ($b) {
                                    return $b['qty_available'] > 0;
                                })
                                ->count();
                            ...
                            $uniquePOs = $batchMetrics->pluck('purchase_order_no')->unique()->count();
                        @endphp

                        <div class="stats-grid"
                            style="grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 16px;">
                            <div
                                style="background: var(--bg-secondary); padding: 12px 16px; border-radius: 12px; border: 1px solid var(--border-color);">
                                <div
                                    style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; color: var(--text-muted); font-weight: 600;">
                                    Total Batches</div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                            ...
                            <div
                                ...
                                <div
                                    style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; color: var(--text-muted); font-weight: 600;">
                                    {{ __('ui.utilization') }}</div>
                                <div style="font-size: 20px; font-weight: 700; color: var(--success);">
                                    {{ number_format($overallUtilization, 1) }}%</div>
                            </div>
                        </div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-top: 12px;">
                            ...
                            <div
                                ...
                                <div
                                    style="font-size: 11px; text-transform: uppercase; letter-spacing: 0.5px; color: var(--text-muted); font-weight: 600;">
                                    Total Value</div>
                                <div style="font-size: 20px; font-weight: 700; color: var(--purple);">
                                    <span class="currency-sm">$</span>{{ number_format($stats['total_value_usd'], 2) }}
                                </div>
                            </div>
                        </div>
                        ...
                        @if ($totalProfitLoss != 0)
                            <div
                                style="margin-top: 16px; padding: 12px 16px; background: var(--bg-secondary); border-radius: 12px; border: 1px solid var(--border-color); display: flex; justify-content: space-between; align-items: center;">
                                <span style="font-weight: 500; color: var(--text-secondary);">
                                    <i class="bi bi-pie-chart"></i> Total Profit/Loss
                                </span>
                                <span
                                    style="font-size: 18px; font-weight: 700; color: {{ $totalProfitLoss > 0 ? 'var(--success)' : 'var(--danger)' }};">
                                    <span class="currency-sm">$</span>{{ number_format(abs($totalProfitLoss), 2) }}
                                    <span
                                        style="font-size: 13px; font-weight: 400; color: var(--text-muted); margin-left: 4px;">
