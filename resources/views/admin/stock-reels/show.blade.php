...
<div class="container-fluid px-3 px-md-4">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-lg-2 col-md-4"><div class="card border-0 shadow-sm"><div class="card-body">
            <div class="text-muted small">Reels</div><div class="fs-4 fw-bold">{{ $summary['reel_count'] }}</div>
        </div></div></div>
        <div class="col-lg-2 col-md-4"><div class="card border-0 shadow-sm"><div class="card-body">
            <div class="text-muted small">Open Remnants</div><div class="fs-4 fw-bold text-warning">{{ $summary['open_count'] }}</div>
        </div></div></div>
        <div class="col-lg-2 col-md-4"><div class="card border-0 shadow-sm"><div class="card-body">
            <div class="text-muted small">Tracked System</div><div class="fs-5 fw-bold">{{ number_format($summary['tracked_system_kg'], 4) }} kg</div>
        </div></div></div>
        <div class="col-lg-2 col-md-4"><div class="card border-0 shadow-sm"><div class="card-body">
            <div class="text-muted small">Measured Reels</div><div class="fs-4 fw-bold">{{ $summary['measured_count'] }}/{{ $summary['reel_count'] }}</div>
        </div></div></div>
        <div class="col-lg-2 col-md-4"><div class="card border-0 shadow-sm"><div class="card-body">
            <div class="text-muted small">Landed Cost</div><div class="fs-5 fw-bold">&#36;{{ number_format($purchaseItem->landedCostPerKg(), 6) }}/kg</div>
        </div></div></div>
    </div>
    ...
    <div class="alert alert-info small">
        ...
        If measured totals differ from the batch balance, use the normal approval-based reconciliation first.
    </div>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-2">
                <div>
                    ...
                    <div class="small text-muted">Reel identity is preserved across production consumption and restores.</div>
                </div>
                @can('update stock')
                <form method="POST" action="{{ route('admin.stock-reels.rebaseline', $purchaseItem) }}">
                    @csrf
                    <button class="btn btn-outline-danger"
                            onclick="return confirm('Re-baseline reel system weights to the latest measurements? This does not change the batch stock total.')">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th>Measured</th>
                            @can('update stock')<th style="min-width:290px;">Record Measurement</th>@endcan
                        </tr>
                    </thead>
                    <tbody>
                    @foreach($summary['reels'] as $reel)
                        ...
                        <tr>
                            ...
                            <td><span class="badge bg-{{ $statusBadge }}">{{ ucfirst($reel->status) }}</span></td>
